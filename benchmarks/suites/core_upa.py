...
CORE_CASES: list[TestCase] = [
    ...
    TestCase("绝对值", "计算 |-15|。", Complexity.SIMPLE, expect_numeric=(15, 0)),
    TestCase("余数计算", "计算 17 除以 5 的余数。", Complexity.SIMPLE, expect_numeric=(2, 0)),

    # ---- Medium Tests ----
    TestCase("列表排序", "将列表 [5, 2, 8, 1, 9, 3] 排序。", Complexity.MEDIUM, expect_pattern=r"\[1,\s*2,\s*3,\s*5,\s*8,\s*9\]"),
    ...
    TestCase("最大公约数", "计算 48 和 18 的最大公约数。", Complexity.MEDIUM, expect_numeric=(6, 0)),
    TestCase("字符串拼接", "将 ['Hello', 'World', 'UPA'] 用空格连接。", Complexity.MEDIUM, expect_contains="Hello World UPA"),
    TestCase("列表求和", "计算 [1, 3, 5, 7, 9] 的和。", Complexity.MEDIUM, expect_numeric=(25, 0)),
    ...
    TestCase("空列表处理", "对空列表 [] 求和。", Complexity.EDGE_CASE, expect_numeric=(0, 0)),
