...
HYBRID_CASES: list[HybridTest] = [
    ...
    HybridTest(
        "翻译后计数",
        "把 'Hello World' 翻译成中文，然后计算翻译结果的字符数量",
    ...
    HybridTest(
        "多语言连接",
        "用 ask_sub_agent 把 'Hello' 翻译成中文，把 'World' 翻译成中文，然后用空格连接输出",
        TaskType.TRANSLATE_LOGIC,
    ...
    HybridTest(
        "摘要后提取关键词",
        "用 ask_sub_agent 总结 'Python是一种广泛使用的高级编程语言'，然后从摘要中提取所有中文词汇的数量",
    ...
    HybridTest(
        "情感分类统计",
        "分析以下句子的情感：'今天太棒了'、'真倒霉'、'还不错'，然后统计积极情感的句子数量",
    ...
    HybridTest(
        "文本摘要评分",
        "总结这段话：'机器学习是人工智能的一个分支。它使计算机能够在没有明确编程的情况下学习。'，然后给摘要打分（每字1分）",
        TaskType.SUMMARIZE_ANALYZE,
    ...
    HybridTest(
        "多句情感平均分",
        "分析这3句：'天气真好'、'太差了'、'很喜欢'。积极=10分，消极=0分。计算3句的平均分（保留整数）",
        TaskType.SENTIMENT_CALC,
    ...
    HybridTest(
        "提取排序",
        "从'分数：85, 92, 78, 95'中提取所有数字，排序后输出",
        TaskType.EXTRACT_PROCESS,
    ...
    HybridTest(
        "链式翻译",
        "用 ask_sub_agent 把'你好'翻译成英文，再用 ask_sub_agent 把结果翻译回中文",
        TaskType.RECURSIVE,
    ...
    HybridTest(
        "嵌套语义调用",
        "用 ask_sub_agent 问：'5乘以3等于几'，打印返回的答案",
        TaskType.RECURSIVE,
    ...
    HybridTest(
        "三级处理链",
        "1) 翻译'Hello'成中文 2) 统计字数 3) 乘以3。只输出最终结果数字",
        TaskType.RECURSIVE,
    ...
    HybridTest(
        "评论分析系统",
        "分析评论'这个产品很棒，但价格有点贵'。规则：包含'很棒'等积极词+40分，包含'贵'等消极词-20分，基础分50分。输出最终评分（如：70）",
        TaskType.SENTIMENT_CALC,
    ...
    HybridTest(
        "多语言处理",
        "将'How are you?'翻译成中文，然后翻译成法语（用子Agent），统计总共调用了几次子Agent",
        TaskType.RECURSIVE,
