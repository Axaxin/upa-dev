...
- safe_sub_agent(query: str): 装饰器，为 ask_sub_agent 调用添加自动重试和错误处理。
  当子代理调用失败时（返回 "[Error: ..."），会自动重试最多 3 次。

  使用方式 1 - 装饰空函数（推荐）:
  ```python
@safe_sub_agent("Translate 'Hello' to Chinese")
def translation(result):
    print(result)

translation()  # 自动处理错误，最多重试 3 次
```

  使用方式 2 - 装饰带参数的函数:
  ```python
@safe_sub_agent("总结这段文字")
def summarize(text, result):
    print(f"摘要: {result}")

summarize("长文本...")  # result 参数会自动填充子代理结果
```
...
重要：
- 推荐：使用 @safe_sub_agent 装饰器处理语义任务，自动处理错误
- 不要输出任何代码块之外的文字
- 只输出 ```python ... ``` 格式的代码"""
...
只输出代码，不要解释。"""
        ...
        # Create safe_sub_agent decorator for sub-agent
        def create_safe_sub_agent_sub(ask_sub_fn):
            def safe_sub_agent(query: str, max_retries: int = 3):
                """Decorator factory that creates a retry wrapper for sub-agent calls."""
                def decorator(func):
...
def execute_code(code: str, client: OpenAI | None = None, provider: ProviderConfig | None = None) -> tuple[str, str]:
    ...
    # Create safe_sub_agent decorator that uses the bound ask_sub_agent
    ...
    if client:
        ...
        safe_sub_agent_decorator = lambda q, max_retries=3: lambda f: f
