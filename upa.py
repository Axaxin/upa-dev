...
输出格式（严格JSON）：
{
  ...
  "required_tools": ["ask_semantic", "web_search"],
  ...
  "logic_steps": [
    {
      ...
      "args": {"query": "搜索关键词"},
      "input_vars": [],
      "output_var": "search_data",
      "description": "搜索相关信息"
    },
    {
      "id": "S2",
      "action": "ask_semantic",
      "args": {"query": "根据搜索结果回答问题。搜索结果：{search_data}"},
      "input_vars": ["search_data"],
      "output_var": "answer",
      "description": "基于搜索结果回答"
    },
    {
      "id": "S3",
      "action": "set_output",
...
logic_steps:
- S1: web_search(query="支架式教学概念最早提出者") → search_data
- S2: ask_semantic(query="根据搜索结果 {search_data} 回答：支架式教学最早由谁提出？只返回选项字母A/B/C/D") → answer
- S3: set_output(value="{answer}") → _final

示例2：纯计算任务
用户问题："计算斐波那契数列第10项"
logic_steps:
- S1: logic (直接计算，无需工具) → result
- S2: set_output(value="{result}") → _final

判断规则：
- 简单问候/闲聊 → intent="simple_chat", complexity="trivial", skip_planning=true
- 纯数学计算 → intent="computation", logic_steps 中无需工具
- 需要外部知识/实时信息 → 必须用 web_search，然后 ask_semantic 分析结果
- 翻译/总结 → intent="semantic", logic_steps 使用 ask_semantic
    ...
    try:
        ...
        for ls_data in data.get("logic_steps", []):
            logic_steps.append(LogicStep(
                ...
                args=ls_data.get("args", {}),
                input_vars=ls_data.get("input_vars", []),
...
2. {var} 表示变量插值，替换为实际变量引用
3. 严禁跳过步骤或忽略变量
4. 所有事实信息必须来源于变量，禁止使用训练数据中的知识

"""

    # Compile logic_steps to pseudo-code
    base_prompt += "【逻辑契约】\n"
