...
2. 评估任务复杂度
3. 确定需要哪些工具（ask_semantic, web_search, safe_semantic）
4. 为复杂任务分解步骤
5. 提供编码提示
...
输出格式（严格JSON）：
{
  ...
  "steps": [
    {"order": 1, "description": "...", "tool_needed": "ask_semantic", "expected_output": "...", "dependencies": []},
...
工具说明：
- ask_semantic: 用于翻译、总结、情感分析等语义任务
- web_search: 用于查询实时信息、事实核查
- safe_semantic: 语法糖装饰器，简化语义调用
...
- 需要翻译/总结 → intent="semantic", required_tools=["ask_semantic"]
- 翻译+计算等混合 → intent="hybrid", required_tools=["ask_semantic"], steps详细分解
